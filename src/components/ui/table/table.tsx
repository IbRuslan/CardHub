...
export const Table = forwardRef<HTMLTableElement, ComponentPropsWithoutRef<'table'>>(
  ({ ...rest }, ref) => {
    return <table className={s.table} {...rest} ref={ref} />
  }
)
...
export const TableHeadCell = forwardRef<ElementRef<'th'>, ComponentPropsWithoutRef<'th'>>(
  ({ children, ...rest }, ref) => {
    return (
      <th className={s.headCell} {...rest} ref={ref}>
        <span>{children}</span>
      </th>
    )
  }
...
export const TableEmpty: FC<ComponentProps<'div'> & { mb?: string; mt?: string }> = ({
  ...
  return (
    <Typography className={s.empty} style={{ marginBottom: mb, marginTop: mt }} variant={'h2'}>
